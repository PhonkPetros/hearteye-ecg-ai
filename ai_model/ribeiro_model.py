...
from tensorflow.keras.layers import Input, Conv1D, MaxPooling1D, BatchNormalization, Activation, Dropout, Add, Flatten, Dense
...
class ResidualUnit:
    def __init__(self, n_samples_out, n_filters_out, kernel_initializer='he_normal', dropout_keep_prob=0.8, kernel_size=17, preactivation=True, postactivation_bn=False, activation_function='relu'):
    ...
    def _skip_connection(self, y, downsample, n_filters_in):
        ...
        if n_filters_in != self.n_filters_out:
            y = Conv1D(self.n_filters_out, 1, padding='same', use_bias=False, kernel_initializer=self.kernel_initializer)(y)
        return y
    ...
    def __call__(self, inputs):
        ...
        x = Conv1D(self.n_filters_out, self.kernel_size, padding='same', use_bias=False, kernel_initializer=self.kernel_initializer)(x)
        x = self._batch_norm_plus_activation(x)
        if self.dropout_rate > 0:
            x = Dropout(self.dropout_rate)(x)
        x = Conv1D(self.n_filters_out, self.kernel_size, strides=downsample, padding='same', use_bias=False, kernel_initializer=self.kernel_initializer)(x)
...
class ECGSequence(tf.keras.utils.Sequence):
    @classmethod
    def get_train_val_test(cls, train_hdf5, val_hdf5, test_hdf5, ecg_dset, labels_dset, batch_size=8, num_samples=None, random_seed=None):
        """
        ...
        with h5py.File(train_hdf5, 'r') as f:
            ...
            train_labels = f[labels_dset][:num_samples] if num_samples else f[labels_dset][:]
        
        with h5py.File(val_hdf5, 'r') as f:
            val_data = f[ecg_dset][:num_samples] if num_samples else f[ecg_dset][:]
            val_labels = f[labels_dset][:num_samples] if num_samples else f[labels_dset][:]
        
        with h5py.File(test_hdf5, 'r') as f:
            test_data = f[ecg_dset][:num_samples] if num_samples else f[ecg_dset][:]
            test_labels = f[labels_dset][:num_samples] if num_samples else f[labels_dset][:]
        ...
        train_seq = cls(train_data, train_labels, train_hdf5, ecg_dset, labels_dset, batch_size=batch_size)
        val_seq = cls(val_data, val_labels, val_hdf5, ecg_dset, labels_dset, batch_size=batch_size)
        test_seq = cls(test_data, test_labels, test_hdf5, ecg_dset, labels_dset, batch_size=batch_size)

        return train_seq, val_seq, test_seq


    def __init__(self, ecg_data, labels_data, file_path, ecg_dset, labels_dset, batch_size=8):
    ...
    def __getitem__(self, idx):
        ...
        # Transpose ECG data
        batch_x = np.transpose(batch_x, (0, 2, 1))  # (batch_size, 12, 5000) -> (batch_size, 5000, 12)
...
def get_model(n_classes, input_shape=(5000, 12)):
    ...
    x = signal
    x = Conv1D(64, 16, padding='same', use_bias=False, kernel_initializer='he_normal')(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
...
def main(args):
    ...
    train_seq, val_seq, test_seq = ECGSequence.get_train_val_test(
        args.train_hdf5, args.val_hdf5, args.test_hdf5, 'ecg_data', 'labels',
        batch_size=args.batch_size, num_samples=args.num_samples, random_seed=random_seed
    )
...
if __name__ == '__main__':
    ...
    parser.add_argument('train_hdf5', type=str, help='Path to the HDF5 file containing train data')
    parser.add_argument('val_hdf5', type=str, help='Path to the HDF5 file containing validation data')
    parser.add_argument('test_hdf5', type=str, help='Path to the HDF5 file containing test data')
