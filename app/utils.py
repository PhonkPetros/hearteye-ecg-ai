...
def analyze_and_plot_12_lead_fixed(wfdb_basename, plot_folder, file_id):
    ...
    try:
        ...
        physionet_features, intervals = extract_physionet_features_fixed(
        ...
        summary = {
            ...
            "intervals": intervals,
            "heart_rate": (
                int(round(60 * fs / np.median(np.diff(rpeaks))))
                if len(rpeaks) > 1
                else None
            ),
...
def generate_signed_url_from_supabase(
    storage_path: str, bucket_name="ecg-data", expires_in=3600
):
    """
...
def get_signed_urls_for_ecg(ecg):
    ...
    signed_wfdb_url = supabase.storage.from_(bucket_name).create_signed_url(
        ecg.wfdb_path, expires_in=3600
    )["signedURL"]
    signed_plot_url = supabase.storage.from_(bucket_name).create_signed_url(
        ecg.plot_path, expires_in=3600
    )["signedURL"]
...
def predict_ecg_classification(
    summary: dict,
    age: int,
    gender: str,
