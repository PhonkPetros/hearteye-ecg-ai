...
# --- Configuration ---
# Path to your CSV file with labels and fiducial points
LABELS_CSV_PATH = r"E:\AiModel\data_splits\val_records.csv"

# Base path to your MIMIC-IV-ECG waveform files directory
# Corrected based on user's file system example
MIMIC_ECG_BASE_DIR = r"G:\Other computers\My Laptop\data\mimic-iv-ecg-data\files" # Corrected this

# Parameters for data preparation
TARGET_SAMPLING_RATE = 500 # Hz
WINDOW_LENGTH_SEC = 10 # Seconds for each training sample
...
OUTPUT_DIR = "processed_ecg_data_val" # Directory to save all processed validation data
...
def prepare_data_for_training(labels_df_subset, ecg_base_dir,
    ...
    for original_index, row in labels_df_subset.iterrows():
        ...
        try:
            ...
            initial_X_len_for_this_record = len(current_X_data_list)
            ...
            if signals is None or signals.size == 0:
                # print(f"DEBUG: No signal data for record {wfdb_path_stem}, skipping.")
                continue

            processed_signals, current_fs = preprocess_ecg_signals(signals, original_fs)

            if processed_signals.shape[0] < WINDOW_LENGTH_SAMPLES:
                # print(f"DEBUG: Signal too short ({processed_signals.shape[0]} samples) for record {wfdb_path_stem}, skipping.")
                continue

            num_windows = processed_signals.shape[0] // WINDOW_LENGTH_SAMPLES

            for i_window in range(num_windows):
                i_window_start = i_window * WINDOW_LENGTH_SAMPLES
                i_window_end = i_window_start + WINDOW_LENGTH_SAMPLES
                ecg_window = processed_signals[i_window_start:i_window_end, :]

                scale_factor_for_fiducials = current_fs / original_fs

                fiducial_points_original_fs_indices = {
                    'p_onset': row['p_onset'],
                    'p_end': row['p_end'],
                    'qrs_onset': row['qrs_onset'],
                    'qrs_end': row['qrs_end'],
                    't_end': row['t_end']
                }
...
if __name__ == "__main__":
    ...
    print(f"Using MIMIC_ECG_BASE_DIR: {MIMIC_ECG_BASE_DIR}")
    print("Loading validation labels CSV...")
    try:
        ...
        print(f"Loaded and initially filtered {len(labels_df_full)} validation records from CSV.")
        if labels_df_full.empty:
            print("Validation CSV is empty or no records with valid IDs and fiducial points. Exiting.")
            exit()
    except FileNotFoundError:
        print(f"Error: Validation CSV file not found at {LABELS_CSV_PATH}")
        exit()
    except Exception as e:
        print(f"Error loading or parsing validation CSV: {e}")
        exit()
    ...
    if os.path.exists(PROGRESS_FILE_PATH):
        try:
            ...
            if processed_X_list or processed_Y_list or processed_info_list:
                 print(f"Loaded {len(processed_X_list)} existing processed validation windows.")
    ...
    if start_original_index >= len(labels_df_full):
        print("All validation records from CSV have already been processed according to progress file.")
        if processed_X_list: 
            ...
            if final_X_data.size > 0: # Check if array is not empty
                print(f"\nFinal loaded validation data shapes: X: {final_X_data.shape}, Y: {final_Y_data.shape}, Info: {final_df_info.shape}")
        exit()

    labels_df_to_process_this_run = labels_df_full.iloc[start_original_index:]

    max_new_records_this_run = None 
    ...
    if labels_df_to_process_this_run.empty:
        print("No new validation records to process in this run based on start_index and max_new_records_this_run.")
        if processed_X_list: 
            final_X_data = np.array(processed_X_list)
            final_Y_data = np.array(processed_Y_list)
            final_df_info = pd.DataFrame(processed_info_list)
            if final_X_data.size > 0:
                 print(f"\nStats for previously processed validation data: X: {final_X_data.shape}, Y: {final_Y_data.shape}, Info: {final_df_info.shape}")
        exit()

    print(f"\nStarting validation data preparation pipeline for {len(labels_df_to_process_this_run)} records (or up to limit).")
    ...
    if final_X_data.size > 0 and final_Y_data.size > 0:
        print(f"\nValidation data preparation pipeline finished.")
        if not final_df_info.empty:
            print(f"Total unique validation ECG records processed across all runs (estimated from df_info): {final_df_info.drop_duplicates(subset=['subject_id', 'study_id', 'cart_id']).shape[0]}")
        else:
            print("No unique validation ECG records information available in df_info.")
        print(f"Total validation ECG windows generated (X_data): {final_X_data.shape}")
        print(f"Total validation Target masks generated (Y_data): {final_Y_data.shape}")
        print(f"Info DataFrame for processed validation windows (df_info): {final_df_info.shape}")

        if not final_df_info.empty and final_X_data.size > 0: # Ensure X_data is not empty for indexing
            print("\nExample of first processed validation record info (overall):")
            print(final_df_info.head(1))
            print("\nExample of first validation ECG window (overall, first lead, first 10 samples):")
            print(final_X_data[0, :10, 0])
            print("\nExample of corresponding validation target mask (overall, first 10 samples, P, QRS, QT):")
            print(final_Y_data[0, :10, :])
    else:
        print("\nNo validation data was ultimately processed or loaded. Check paths, file existence, and logs for errors.")

    num_records_in_csv_slice = len(labels_df_to_process_this_run)
    print(f"\nAttempted to process {num_records_in_csv_slice} validation records in this specific run (from the current slice of the CSV).")
    if not final_df_info.empty:
        num_unique_successfully_processed_total = final_df_info.drop_duplicates(subset=['subject_id', 'study_id', 'cart_id']).shape[0]
        print(f"{num_unique_successfully_processed_total} unique validation records have resulted in processed data across all runs.") 