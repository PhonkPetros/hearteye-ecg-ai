...
def batch_extract_features_from_ecg(wfdb_root_folder, output_csv, plot_folder="plots", max_new=10000):
    """
            ...
            if file.endswith(".hea"):
                ...
                try:
                    ...
                    row = {
                        ...
                        'p_duration': (features.get('p_end') - features.get('p_onset')) if features.get('p_end') is not None and features.get('p_onset') is not None else None,
                        'pq_interval': features.get('qrs_onset') - features.get('p_onset') if features.get('qrs_onset') and features.get('p_onset') else None,
                        'qrs_duration': features.get('qrs_end') - features.get('qrs_onset') if features.get('qrs_end') and features.get('qrs_onset') else None,
                        'qt_interval': features.get('t_end') - features.get('qrs_onset') if features.get('t_end') and features.get('qrs_onset') else None,
                        'p_axis': features.get('p_axis'),
                        'qrs_axis': features.get('qrs_axis'),
                        't_axis': features.get('t_axis')
                    }

                    if row['heart_rate'] is not None and row['qrs_duration'] is not None:
                        ...
                        print(f"⚠️ Skipping {file_id}: missing heart rate or QRS duration")
